...
num_reads_text <- si(num_reads, acc = 1) #system2("bin/siformat.sh", args = num_reads, stdout = T)
num_bases <- df %>% dplyr::summarise(bases = sum(bases)) %>% as.numeric()
num_bases_text <- si(num_bases, acc = 1) #system2("bin/siformat.sh", args = num_bases, stdout = T)
```
