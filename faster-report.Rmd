...
stats <- parallel::mcmapply(stats_command, fastqfiles, mc.cores = numcores)
...
num_reads <- df %>% dplyr::summarise(seqs = sum(num_seqs)) %>% as.numeric()
...
This is `r params$sequencer` sequencing data. The report contains **`r num_files`** fastq files with a total of **`r num_reads_text`** reads and **`r num_bases_text`** bases.
...
#
# create rawdata dir if not exists before function calls, they write there
if(params$rawdata) {
  if(!dir.exists("rawdata")) {dir.create("rawdata")}  
}

message(paste0('Running faster/fastkmers with ', numcores, ' cores...'))
gc_density <- parallel::mcmapply(faster_gc, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
q_score_density <- parallel::mcmapply(faster_qscore, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
kmers_tbl_list <- parallel::mcmapply(fastkmers, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
# do length only for ONT or PacBio
if (params$sequencer != "Illumina") {
  len_density <- parallel::mcmapply(faster_len, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
}

# do duplevel only for Illumina
if(params$sequencer == "Illumina") {
  
  duplevel_tbl <- parallel::mcmapply(duplevel, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
  content_tbl <- parallel::mcmapply(content_percycle, fastqfiles, SIMPLIFY = FALSE, MoreArgs = list(saveraw = params$rawdata))
}
