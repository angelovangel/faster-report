...
tagList(
  ...
  reactable::reactable(
    ...
    columns = list(
      ...
      min_len = colDef(format = colFormat(separators = T)),
      max_len = colDef(format = colFormat(separators = T)),
      N50 = colDef(format = colFormat(separators = T)),
      Q30 = colDef(format = colFormat(suffix = '%', digits = 2))
    )
  )
)
...
gc_df %>%
  dplyr::arrange(stringi::stri_rank(file, opts_collator = locale)) %>%
  reactable(
    fullWidth = T, pagination = F,
    defaultColDef = colDef(html = T, sortable = F, filterable = F, resizable = T),
    columns = list(
